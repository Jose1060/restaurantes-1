...
const resolvers = {
	Query: {
		...
		getRestaurantesCerca: async (_, { latitud1, longitud1, etiquetas2 }) => {
			...
			const rest2 = await restaurantes.find({
				$and: [
					{ longitud: { $gte: y1, $lte: y2 } },
					{ latitud: { $gte: x1, $lte: x2 } },
					{ etiquetas: { $in: etiquetas2 } },
				],
			});
	...
	Mutation: {
		...
		createCalificacion: async (_, args) => {
			...
			if (rest.N_calificaciones == 0.0) {
				const updatecal = await restaurantes.findOneAndUpdate(
					idRest,
					{
				...
				const updateprom = await restaurantes.findOneAndUpdate(
					idRest,
					{
				...
				const updatetotalPunto = await restaurantes.findOneAndUpdate(
					idRest,
					{
				...
				const prom =
					(rest.total_puntos + Calificacion.calificacion) /
					(rest.N_calificaciones + 1);
				...
				const updatetotalPunto = await restaurantes.findOneAndUpdate(
					...
					{
						$set: {
							total_puntos: rest.total_puntos + Calificacion.calificacion,
						},
					},
					{ new: true }
				);
		...
		crearRestaurante: async (_, args) => {
			...
			const newRestaurante = new restaurantes({
				nombre,
				direccion,
				telefono,
				email,
		...
		async updateRestaurante(_, { Restaurante, id }) {
			const restupdate = await restaurantes.findByIdAndUpdate(
				id,
				{
					$set: Restaurante,
				},
